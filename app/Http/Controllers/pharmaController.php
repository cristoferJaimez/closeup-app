...
*/
        ...
        //
        $res = DB::select('select * from phar LEFT JOIN neighborhoods on neighborhoods.co_barrio=phar.utc LEFT JOIN geo_utc on geo_utc.barrio_id = 	neighborhoods.id LEFT JOIN municipalities on municipalities.id = geo_utc.municipio_id LEFT JOIN departments on departments.id = geo_utc.departamento_id  WHERE municipalities.municipio LIKE ? OR departments.departamento LIKE ? OR neighborhoods.desc_utc LIKE ? ', [$request->google,$request->google,$request->google]);
        return $res;
